      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-bg-base">
            <th className="text-center text-text-muted font-bold uppercase tracking-wider text-[11px] px-3 py-3 w-10">#</th>
            <th className="text-left text-text-muted font-bold uppercase tracking-wider text-[11px] px-3 py-3">Team</th>
            <th className="text-center text-text-muted font-bold uppercase tracking-wider text-[11px] px-3 py-3 w-10">P</th>
            <th className="text-center text-text-muted font-bold uppercase tracking-wider text-[11px] px-3 py-3 w-10">W</th>
            <th className="text-center text-text-muted font-bold uppercase tracking-wider text-[11px] px-3 py-3 w-10">D</th>
            <th className="text-center text-text-muted font-bold uppercase tracking-wider text-[11px] px-3 py-3 w-10">L</th>
            <th className="text-center text-text-muted font-bold uppercase tracking-wider text-[11px] px-3 py-3 w-10">A</th>
            <th className="text-center text-text-muted font-bold uppercase tracking-wider text-[11px] px-3 py-3 w-12">GD</th>
            <th className="text-center text-accent font-bold uppercase tracking-wider text-[11px] px-3 py-3 w-12">Pts</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row: any, index: number) => {
            ...
            return (
              ...
              >
                <td className="text-center font-bold text-text-muted px-3 py-3">{index + 1}</td>
                <td className="px-3 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    {qualifies && (
                      <span className="text-[9px] font-black text-accent border border-accent/30 rounded px-1.5 py-0.5">Q</span>
                    )}
                  </div>
                </td>
                <td className="text-center text-text-secondary px-3 py-3">{row.played ?? 0}</td>
                <td className="text-center text-text-secondary px-3 py-3">{row.wins ?? 0}</td>
                <td className="text-center text-text-secondary px-3 py-3">{row.draws ?? 0}</td>
                <td className="text-center text-text-secondary px-3 py-3">{row.losses ?? 0}</td>
                <td className={`text-center px-3 py-3 ${(row.absent ?? 0) > 0 ? 'text-orange-400 font-medium' : 'text-text-secondary'}`}>{row.absent ?? 0}</td>
                <td className={`text-center font-semibold px-3 py-3 ${gd >= 0 ? 'text-feedback-success' : 'text-feedback-error'}`}>{gd > 0 ? `+${gd}` : gd}</td>
                <td className="text-center font-black text-accent px-3 py-3">{row.points ?? 0}</td>
              </tr>
            )
          })}
        </tbody>
              ...
              {Object.entries(groupStandings)
                ...
                .map(([groupName, rows]) => (
                  <div key={groupName} className="space-y-3">
                    <h3 className="text-sm font-black uppercase tracking-wider text-accent">{formatGroupTitle(groupName)}</h3>
