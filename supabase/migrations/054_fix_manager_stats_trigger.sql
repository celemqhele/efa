-- Fix manager tenure stats not updating in real time.
--
-- Problem: on_result_for_manager_stats (AFTER INSERT/UPDATE on results) fires
-- alphabetically BEFORE on_result_insert, which is what flips fixtures.status to
-- 'confirmed'. The old recalc_tenure_stats filtered on
-- f.status IN ('confirmed','abandoned_home','abandoned_away','abandoned_both'),
-- so at trigger time the fixture was still 'awaiting_confirmation' and every
-- match was dropped -> tenure stats written as 0 and never re-run.
--
-- Fix: drop the fixture-status filter. A results row only exists for a
-- finalised/abandoned match, so the join to results is sufficient and the
-- recalc no longer depends on trigger ordering.
--
-- Also add an AFTER DELETE handler so voiding/resetting a result (admin
-- reset-fixture route, WhatsApp void path) recalculates tenure stats instead
...
-- AFTER DELETE handler: recompute tenures for the teams of the removed result.
CREATE OR REPLACE FUNCTION trigger_recalc_on_result_delete()
RETURNS TRIGGER
LANGUAGE plpgsql SECURITY DEFINER
AS $$
DECLARE
...
BEGIN
  SELECT * INTO v_fixture FROM fixtures WHERE id = OLD.fixture_id;
  IF NOT FOUND THEN
    RETURN OLD;
  END IF;
...
DROP TRIGGER IF EXISTS on_result_delete_for_manager_stats ON results;
CREATE TRIGGER on_result_delete_for_manager_stats
  AFTER DELETE ON results
  FOR EACH ROW
  EXECUTE FUNCTION trigger_recalc_on_result_delete();
