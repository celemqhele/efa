/**
 * One-off: submit the return-leg result — Nantes (home) 1 – 3 Al Ettifaq (away),
 * fixture c31b254d-f624-4cdd-ab20-e9eb82165f64 (EFA Champions League MD41, Group B).
 *
 ...
 *   and scores predictions; it does NOT confirm group fixtures, so status is
 *   updated here — same as the app's fallback update)
 * - voids pending backdoor submissions for the fixture
 * - notifies both team managers in-app
 * - writes an audit_log entry
 *
 * Run: npx tsx scripts/submit-nantes-al-ettifaq-result.ts
 */
...
const FIXTURE_ID = 'c31b254d-f624-4cdd-ab20-e9eb82165f64'
const HOME_SCORE = 1
const AWAY_SCORE = 3
...
async function main() {
  ...
  if (statusError || updatedFixture?.status !== 'confirmed') {
    console.log('Fixture not auto-confirmed (expected for group round) — updating status to confirmed.')
    const { error: confirmError } = await supabase
      .from('fixtures')
      .update({ status: 'confirmed' })
      .eq('id', FIXTURE_ID)
    if (confirmError) {
      console.error('[submit-result] failed to confirm fixture:', confirmError.message)
      process.exit(1)
    }
